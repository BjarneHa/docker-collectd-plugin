    ...
    def read(cls, container, stats, t):
        ...
        if 'precpu_stats' in stats:
            ...
            if system_delta > 0 and cpu_delta > 0:
                cpu_percent = 100.0 * cpu_delta / system_delta * num_cpus
...
class NetworkStats(Stats):
    @classmethod
    def read(cls, container, stats, t):
        for interface, if_stats in stats["networks"].items():
            items = sorted(if_stats.items())
            cls.emit(container, 'network.usage', [x[1] for x in items],  type_instance=interface, t=t)
